...
const DashboardOrangtua = () => {
  ...
  useEffect(() => {
    ...
    const fetchTugas = async () => {
      try {
        const response = await fetch('http://localhost:5000/orang-tua/tugas', {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await response.json();
    ...
    const fetchJadwal = async () => {
      try {
        const response = await fetch("http://localhost:5000/orang-tua/jadwal-hari-ini", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await response.json();
        ...
        <div className="row mb-4">
          <div className="col-md-8">
            <div className="bg-white d-flex justify-content-between align-items-center p-4 rounded shadow-sm h-100">
              <div>
                <h5 className="fw-bold">Hallo {profil?.orang_tua?.nama_lengkap || '...'}</h5>
                <p className="text-muted">
                  Anda dapat memantau perkembangan akademik anak Anda di sini.
                </p>
              </div>
                ...
                {tugas.length === 0 ? (
                  ...
                  tugas.map((item, index) => (
                    <li key={index}>
                      ...
                      {item.judul} - <span className="text-muted">{item.tanggal_deadline?.slice(0, 10)}</span>
                    </li>
                  ))
                )}
              </ul>
              ...
              <div className="small">
                ...
                <ul className="ps-3">
                  <li>
                    ...
                    Pengambilan rapor tanggal 29
                  </li>
                  <li>
                    <i className="bi bi-exclamation-circle-fill text-danger me-2"></i>
                    Libur akhir semester dimulai minggu depan
                  </li>
                </ul>
              </div>
            </CardBox>
